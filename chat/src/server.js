...
pool.connect((err, clent, release) => {
    if (err) {
        return console.error("Databasega ulanishda xato yuz berdi....");
    } else {
        release();
...
// user A => user B  => messages save
// user A => user C =>
// localhost:5000/message:/userId1:/userId2
...
io.on("connection", (socket) => {
    ...
    socket.on("user:join", async (userId) => {
        ...
        console.log(`👤 User ${userId} online bo'ldi`);
